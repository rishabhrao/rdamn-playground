...
import { SIGNED_PUT_URL, UserDir } from "../constants"
...
const saveToS3: () => Promise<void> = async () => {
	...
	execSync(`cd ${UserDir} && rm -rf ./rdamn-template-saved ./code.zip && mkdir ./rdamn-template-saved && cp -rf ./code/* ./rdamn-template-saved/`)

	// Create zip file with new code and store it as `code.zip`
	execSync(`cd ${UserDir} && zip -r -9 ./code.zip ./rdamn-template-saved -x ${ignoredDirs.map(dir => `*${dir}*`).join(" ")}`)
	...
	const s3PutResponse = await fetch(SIGNED_PUT_URL || "", {
		...
		body: readFileSync(`${UserDir}/code.zip`),
	})
	...
	// Cleanup
	execSync(`cd ${UserDir} && rm -rf ./rdamn-template-saved ./code.zip`)
}
